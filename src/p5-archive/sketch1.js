const sketch1 = (p) => {
    ...
    const drawDistricts = () => {
        ...
        for(let i=0; i<1; i++) {
            ...
            if(dtSelRandom == 0) {
                ShapesArr.push([dtLeft + 1, dtTop + 1, dtSize - 2, dtSize - 2, genShape[2]*.01*dtSize, genShape[3]*.01*dtSize, genShape[4]*.01*dtSize, genShape[5]*.01*dtSize, genShape[0], genShape[1], 0, 0])
            } else {
    ...
    p.draw = () => {
        ...
        if(progressMoving >= 0 && progressMoving <= 1 && accessMoving) {
            ...
            ShapesToMoveArr.map((key, _i) => {
                ...
                let optionShape = key.options[queueCommon]
